...
```
description --> Jev: 44 questions --> DashboardSpec --> tools --> A2UI --> ACTIVITY_SNAPSHOT
```
...
| The order of the tiles | one Choice over the positions `1` to `20` per tile type, and one for the opposite direction |

All 44 questions go out in **one** request, the
[speculative fan-out](https://docs.typesafe.ai/patterns/fan-out) the docs
recommend: about 7,900 input tokens and 0.4 to 1 second. The code reads the
...
The tiles appear in the order in which the description asks for them. Every tile
type gets one more Choice, over the positions `1` to `20`: *at which position
does it ask for this tile?* A Choice always names some position, so it cannot say
whether the tile is wanted at all, and the tile's Noul keeps doing that. The
code sorts the wanted tiles by their position.

The Jev docs warn that it
[does not count reliably](https://docs.typesafe.ai/model-jaggedness/jev-1.13), and
that the error grows with the length of the list. For the handful of tiles a
dashboard has, it held: in the 11 test descriptions with more than one tile, the
order came out right every time, inside a single sentence and in German too. Two
details make it robust where the count is shaky:

- The position of a tile is not the option Jev picked but the sum of all
  positions weighted with their probabilities. Two tiles that both come out as
  `3` still differ when one of them leans towards `4`.
- The copies for the opposite direction go where the sentence that asks for them
  stands, minus half a position. Whether that sentence counts as a position of
  its own only changes the count of the tiles after it, never of those before it.
  A tie with it can therefore only come from a tile that follows.

### What the wording taught

The questions were tried on 18 descriptions: the two examples of the page, terse
prose, German, missing and unsupported cities, single tiles, a question that
has nothing to do with dashboards, and four whose tiles come in an unusual order,
one of them inside a single sentence. Three changes made the difference, and all
...
Closed questions also set a limit. Every tile type appears at most once per
direction, so two flight tables for unrelated routes cannot be expressed.
